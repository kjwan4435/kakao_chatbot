...
sleepRouter.route("/1").post((req, res) => {
  ...
  if (answer === "오늘 기분 너무 안 좋아..") {
    ...
    sentence =
      "그러셨군요! 오늘은 주인님의 최근 잠자리에 대해서 여쭤볼려고 해요!";
    ...
    template: {
      outputs: [
        ...
        },
        {
          simpleText: {
            text:
              "주인님의 최근 일주일동안의 수면 만족도는 어느정도인가요?\n\n1. 매우 불만족\n2. 다소 불만족\n3. 보통\n4. 다소 만족\n5. 매우 만족"
          }
        }
      ],
      quickReplies: [
...
sleepRouter.route("/4").post((req, res) => {
  const question = req.body.userRequest.block.name;
  const blockid = req.body.userRequest.block.id;
  const answer = req.body.userRequest.utterance;
  const id = req.body.userRequest.user.id;
    ...
    template: {
      outputs: [
        ...
        },
        {
          simpleText: {
            text:
              "주인님은 잠이 우울에 가장 큰 영향을 준다는 사실 알고 계셨나요?\n\n우울하거나 기분전환이 필요할 때는 잠을 푹 자고 다음 날을 힘차게 시작하는 것이 최고라구요!"
          }
        },
        {
          simpleText: {
